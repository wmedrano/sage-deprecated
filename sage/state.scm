...
(define* (run! tui)
  ...
  (quit!)
  (set! %tui tui)
  (define (maybe-handle-frame-resize! frame-size)
    (when (and frame-size (not (equal? frame-size %tui-frame-size)))
      (define frame-width (assoc-ref frame-size 'width))
      (define frame-height (assoc-ref frame-size 'height))
      (set! %tui-frame-size frame-size)
      (run-hook resize-hook frame-width frame-height)))
  (define (handle-all-events)
    (let handle-all-events ((events (internal:events-from-terminal)))
      (for-each (lambda (e)
                  (define window %focused-window)
                  (when window
                    (define buffer (window:window-buffer window))
                    (run-hook (buffer:buffer-event-hook buffer)
                              window buffer e)))
                events)
      (if (pair? events) (handle-all-events (internal:events-from-terminal)))))
  (define (draw)
    (define frame-size (and %tui (tui:tui-draw! %tui (windows))))
    (maybe-handle-frame-resize! frame-size))
  (define (run-loop-iteration!)
    (handle-all-events)
    (draw))
  (define (run-main-loop)
    (maybe-handle-frame-resize! (tui:tui-draw! %tui (windows)))
    (while %tui
      (run-loop-iteration!))
    (quit!))
